...
def extract_electricity_mix(updated_project_name: str, output_dir: str, results_filename: str,
    ...
    print('extracing electricity mix',flush=True)
    print('project_name ',updated_project_name,flush=True)
    bw.projects.set_current(updated_project_name)
    ei_cf_36_db = bw.Database(updated_database)
    print('extracted database:', updated_database,flush=True)
    elec_data = {
        'elec_type': [],
        'elec_amount kwh': [],
        'elec_loc': [],
    ...
    print('Starting for loop for searching for electricity processes',flush=True)
    ...
    print('Searched and saved dataframe',flush=True)
    ...
    '''
    print('extracing farming and supply of rapeseed',flush=True)
    print('project_name ',updated_project_name,flush=True)
    bw.projects.set_current(updated_project_name)
    ei_cf_36_db = bw.Database(updated_database)
    print('extracted database:', updated_database,flush=True)
    ...
    print('Starting for loop for searching for farming and supply of rapeseed',flush=True)
    for act in ei_cf_36_db:
        if (act['name'] == 'farming and supply of rapeseed') and act['location'] == 'USA':
            for exch in act.exchanges():
                    elec_data['process'].append(act['name'])
                    elec_data['flow'].append(exch['name'])
                    elec_data['amount'].append(exch['amount'])
                    elec_data['unit'].append(exch['unit'])
                    elec_data['database'].append(updated_database)
    print('Searched and saved dataframe',flush=True)
    elec_df = pd.DataFrame(elec_data)

    if not elec_df.empty:
        elec_df.to_csv(output_dir + updated_database + '_farming and supply of rapeseed.csv', index=False)
    '''
