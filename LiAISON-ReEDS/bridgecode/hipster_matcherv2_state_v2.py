...
def edit_ecoinvent():
    ...
    #All regions of RoW, GLO and RER. Not sure how it used
    ...
    #Only US based activities belonging to power sector. 
...
def process_inv(year1,scenario_base,reeds_output_raw):
    ...
    #Getting states for BA regions and replacing BA with State to make the data size small and improve time
    region_bridge = pd.read_excel('ReEDS_State.xlsx')
    reeds_output_raw = reeds_output_raw.merge(
            region_bridge, left_on=['BA'], right_on=['Region']).drop_duplicates()

    reeds_output_raw['BA'] = reeds_output_raw['State']
    del reeds_output_raw['State']
    del reeds_output_raw['Region']
    del reeds_output_raw['NERC']
    #Clean file
    ...
    #Bridge file matching ReEDS location to State regions in the United States
    #region_bridge = pd.read_excel('ReEDS_State.xlsx')
    #reeds_df_restructure = reeds_df_restructure.merge(
    #    region_bridge, left_on=['BA'], right_on=['State'])
    #del reeds_df_restructure['BA']
    reeds_df_restructure['State'] = reeds_df_restructure['BA']
    reeds_df_restructure['Region'] = reeds_df_restructure['BA']
    ...
    tech_fuel_df = reeds_df_restructure[[
        'ReEDS_Tech', 'Region', 'State', 'Ecoinvent_process_to_be_created','Year']].drop_duplicates()
    ...
    for index, row in tech_fuel_df.iterrows():
        ...
        data_from_reeds2 = data_from_reeds[['ReEDS_Tech','Region','State','Year','Ecoinvent_process_to_be_created']].drop_duplicates()
        ...
        ecoinvent_data = ecoinvent38_corr.merge(data_from_reeds3,left_on=['process','process location'],right_on=['EcoInvent_Tech','State'])
        ...
        if ecoinvent_data.empty:
            ...
            if ecoinvent_data.empty:
                ...
                if not ecoinvent_data.empty:
                    print('Using RoW RER GLO information for ',row['Ecoinvent_process_to_be_created'], row['Region'], row['State'], row['Year'])
    
            else:
                
                
                if "USA" not in list(ecoinvent_data['process location']):
                    print('Using State regions ',row['Ecoinvent_process_to_be_created'], row['Region'], row['State'], row['Year'])
    
                else:
                    #print('Using all all State regions ',row['Ecoinvent_process_to_be_created'], row['Region'], row['State'], row['Year'])
                    pass
        ...
        if not ecoinvent_data.empty:
                ...
                try:
                    ...
                    ecoinvent_data_f = ecoinvent_data.groupby(['process','ReEDS_Tech', 'Region', 'Year', 'State','flows', 'type_of_flow',
                    ...
                    ecoinvent_data_f1 = ecoinvent_data_f.groupby(['ReEDS_Tech', 'Region', 'Year', 'State','flows', 'type_of_flow',
            ...
            #print(row['Ecoinvent_process_to_be_created'], row['Region'], row['State'], row['Year'])
            #print('Technology does not exist!!!!')
            
            # opening the file in write only mode
            f = open(scenario+"technologiesdonotexist.txt", "a")
            # f is the File Handler
            text=row['Ecoinvent_process_to_be_created']+ row['Region']+ row['State']+ str(row['Year'])
            f.write(text)
            pass
...
def state_grid_mix(year1,scenario_base):

        #Grid mix for US by combining the regional grid mixes
        scenario = scenario_base + str(year1)
        #Provide the ReEDS scenario being studied over here
        ...
        output['process'] = 'ReEDS_State_Grid_Mix'
        ...
        output3 = output2.groupby(['process', 'flow', 'unit', 'input', 'year', 'type', 'process_location','supplying_location','comments'])['value'].agg('sum').reset_index()
        output2 = output3
        ...
        for loc in loc_list:
            ...
            production_df['process'] = ['ReEDS_State_Grid_Mix']
...
def regional_grid_mix_inv(year1,scenario_base):
    ...
    output = reeds_productioneration_mix_input
    output['type_of_flow'] = 'technosphere'
    output['location'] = output['State'] +'-'+output['Region']
    output['input'] = 'FALSE'
    output['input'].loc[output['type_of_flow'] == 'technosphere'] = 'TRUE'
    output['process_location'] = output['location']
    output['supplying_location'] = output['location']
...
def us_grid_mix_inv(year1,scenario_base):
...
def joiner_db(scenario,*args):
        df4 = pd.DataFrame()
        for i in args:
            df4 = pd.concat([df4,i])
        
        df4.to_csv('./reedsdata/'+scenario+'_reeds_data_state.csv',index=False)
...
for scenario_base in scen_names:
    reeds_output_raw = pd.read_csv('./ReEDS_postprocessing/Modified_ReEDS_Outputs/'+scenario_base+'_output_BA.csv')
    #reeds_output_raw = pd.read_csv('./climatetarget.csv')
    for year1 in years:

        A=process_inv(year1,scenario_base,reeds_output_raw)
        B=state_grid_mix(year1,scenario_base)
        C=regional_grid_mix_inv(year1,scenario_base)
        D=us_grid_mix_inv(year1,scenario_base)
        scenario = scenario_base + str(year1)
        joiner_db(scenario,A,B,D)
