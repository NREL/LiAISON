...
from liaison.lca_calculator_automated import brightway,lcia_traci_run,lcia_recipe_run, lcia_premise_gwp_run
...
def main_run(lca_project,updated_project_name,initial_year,results_filename,mc_foreground_flag,lca_flag,lca_activity_modification,regional_sensitivity_flag,region,data_dir,primary_process,process_under_study,location_under_study,functional_unit,updated_database,mc_runs,inventory_filename,modification_inventory_filename,process_name_bridge,emission_name_bridge,location_name_bridge,output_dir,bw):
    ...
    def lca_runner(db,r,mc_runs,mc_foreground_flag,lca_flag):
            ...
            if lca_activity_modification:
               dictionary_modified = brightway_modifier(db,modification_inventory_filename,mc_foreground_flag,mc_runs,process_name_bridge,emission_name_bridge,location_name_bridge,bw)
               print('Activity modified and saved success',flush=True)
            ...
            if lca_flag: 
                result_dir1,n_lcias1 = lcia_traci_run(db,dictionary[process_under_study+'@'+location_under_study],float(functional_unit),mc_foreground_flag,mc_runs,bw)
                result_dir2,n_lcias2 = lcia_recipe_run(db,dictionary[process_under_study+'@'+location_under_study],float(functional_unit),mc_foreground_flag,mc_runs,bw)
                result_dir3,n_lcias3 = lcia_premise_gwp_run(db,dictionary[process_under_study+'@'+location_under_study],float(functional_unit),mc_foreground_flag,mc_runs,bw)
