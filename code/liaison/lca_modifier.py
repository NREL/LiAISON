...
def brightway_modifier(db,run_filename,mc_foreground_flag,mc_runs,process_name_bridge,emission_name_bridge,location_name_bridge,bw):
        ...
        for key in process_dict:
            ...
            for index,row in inp.iterrows():
                ...
                if process_bridge.empty or location_bridge.empty:
                    ...
                    print('Did not find this in the matching file\n',flush = True)
        ...
        def find_emission(emission_bridge):        
            biosphere = bw.Database('biosphere3')
            for i in biosphere:
               y = i.as_dict()
               emission = None
               try: 
                    if y['name'] == emission_bridge['Ecoinvent_name'][0] and y['categories'][1] == emission_bridge['Ecoinvent_source'][0]:
                        emission = i
                        try:
                            emission['location']
                        except:
                            emission['location'] = None
                        try:
                            emission['product']
                        except:
                            emission['product'] = None
                        #print(emission.as_dict())

                        emission['type'] ='biosphere' 
                        return emission
               except:
                    pass
        ...
        for key in process_dict:
            ...
            for index,row in inp.iterrows():
                ...
                emission = find_emission(emission_bridge)
                ...
                if emission == None:
                    ...
                    process_dict[key].new_exchange(input=emission.key,amount=row['value'], name = emission['name'], location = emission['location'],unit=emission['unit'],type='biosphere').save()
                    process_dict[key].save()  
